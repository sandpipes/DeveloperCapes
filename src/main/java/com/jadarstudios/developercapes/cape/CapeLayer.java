...
public class CapeLayer implements LayerRenderer<AbstractClientPlayer> {
	...
	public void doRenderLayer(AbstractClientPlayer entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
        if (entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible() && entitylivingbaseIn.isWearing(EnumPlayerModelParts.CAPE)) {
            ...
            User user = manager.getUser(entitylivingbaseIn.getUniqueID().toString());
            if (user == null) return;

            ICape cape = user.capes.get(0);
            if (cape == null) return;

            if (!cape.isTextureLoaded(entitylivingbaseIn))
                cape.loadTexture(entitylivingbaseIn);
        	
        	ItemStack itemstack = entitylivingbaseIn.getItemStackFromSlot(EntityEquipmentSlot.CHEST);

            if (itemstack == null || itemstack.getItem() == Items.ELYTRA) return;
